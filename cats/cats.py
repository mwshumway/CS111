    ...
    def in_topic(paragraph):
        for e in topic:
            for w in paragraph.split():
                if remove_punctuation(w.lower()) == e:
                    return True
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        d = {word: diff_function(typed_word, word, limit) for word in valid_words}
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    d = {'id': user_id, 'progress': correct / len(prompt)}
    upload(d)
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i])):
            if j < (len(times_per_player[i]) - 1):
                time_diff.append(times_per_player[i][j+1] - times_per_player[i][j])
        d.append(time_diff)
    return match(words, d)
    # END PROBLEM 9


# p = [[75, 81, 84, 90, 92], [19, 29, 35, 36, 38]]
# print(time_per_word("whatever", p))
...
def fastest_words(match):
    ...
    # make dictionary space for each player
    zoom = {}
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if fastest[0] is None or time(match, j, i) < fastest[0]:
                fastest = time(match, j, i), j

        # if fastest[1] not in zoom:
        #     zoom[fastest[1]] = []

        zoom[fastest[1]].append(word_at(match, i))

    cout = []
    zoom = dict(sorted(zoom.items()))
    for key in zoom.keys():
        cout.append(zoom[key])
